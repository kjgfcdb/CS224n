        ...
        for i in range(len(sentence)):
            ...
            for j in range(-window_size, window_size+1):
                if j+i < 0:
                    ...
                    temp.extend(sentence[j+i])
            windowed_data.append((temp, labels[i]))
	### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(
            embeddings, shape=(-1, self.config.n_window_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.get_variable(name="b1", shape=(
            1, self.config.hidden_size), initializer=tf.contrib.layers.xavier_initializer())
        ...
        b2 = tf.get_variable(name="b2", shape=(
            1, self.config.n_classes), initializer=tf.contrib.layers.xavier_initializer())
