    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            U_r = tf.get_variable(name="U_r", shape=(
                self.state_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer())
            W_r = tf.get_variable(name="W_r", shape=(
                self.input_size, self.state_size), initializer=tf.contrib.layers.xavier_initializer())
            b_r = tf.get_variable(name="b_r", shape=(
                self.state_size,), initializer=tf.contrib.layers.xavier_initializer())
