...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(
            embeddings, shape=(-1, self.config.n_features * self.config.embed_size))
        # END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        # YOUR CODE HERE
        W = tf.get_variable(name="W", shape=(self.config.n_features * self.config.embed_size,
                                             self.config.hidden_size), initializer=xavier_weight_init())
        b1 = tf.get_variable(name="b1", shape=(
            1, self.config.hidden_size), initializer=tf.zeros_initializer())
        U = tf.get_variable(name="U", shape=(
            self.config.hidden_size, self.config.n_classes), initializer=xavier_weight_init())
        b2 = tf.get_variable(name="b2", shape=(
            1, self.config.n_classes), initializer=tf.zeros_initializer())
    ...
    def add_loss_op(self, pred):
        ...
        """
        # YOUR CODE HERE
        ce_loss = tf.nn.softmax_cross_entropy_with_logits(logits=pred,
