...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_parses = [(i, partial_parse)
                         for i, partial_parse in enumerate(partial_parses)]
    dependencies = [[] for i in range(len(sentences))]
    while len(unfinished_parses) != 0:
        minibatch = unfinished_parses[:batch_size]
        parses = [pair[1] for pair in minibatch]
        idxs = [pair[0] for pair in minibatch]
        transitions = model.predict(parses)
        finished_parses = []
        for i in range(len(parses)):
            parses[i].parse_step(transitions[i])
            if len(parses[i].stack) == 1 and len(parses[i].buffer) == 0:
                dependencies[idxs[i]] = parses[i].dependencies
                finished_parses.append((idxs[i], parses[i]))
        for parse in finished_parses:
