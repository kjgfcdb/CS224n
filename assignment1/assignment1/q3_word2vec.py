...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    for ctxtword in contextWords:
        c_cost, c_grad_in, c_grad_out = word2vecCostAndGradient(
            inputVectors[tokens[currentWord]], tokens[ctxtword], outputVectors, dataset)
        cost += c_cost
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    # YOUR CODE HERE
    context_idx = [tokens[word] for word in contextWords]
    vhat = inputVectors[context_idx].sum(axis=0)
    cost, grad_predicted, gradOut = word2vecCostAndGradient(
        vhat, tokens[currentWord], outputVectors, dataset)
    for idx in context_idx:
        gradIn[idx] += grad_predicted[idx]
    # END YOUR CODE
    return cost, gradIn, gradOut
