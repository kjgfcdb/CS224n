...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        temp = []
        tail = max_length - len(sentence)
        temp.append(sentence[:max_length]+[zero_vector]*tail)
        temp.append(labels[:max_length]+[zero_label]*tail)
        temp.append(([True]*len(sentence))[:max_length]+[False]*tail)
        ret.append(temp)
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(dtype=tf.int32, shape=(
            None, self.max_length, Config.n_features))
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(
            pre_embeddings, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        U = tf.get_variable(name="U", shape=(
            Config.hidden_size, Config.n_classes), initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable(name="b2", shape=(1, self.config.n_classes), initializer=tf.constant_initializer())
        state = tf.zeros((1, Config.hidden_size))
        ### END YOUR CODE
